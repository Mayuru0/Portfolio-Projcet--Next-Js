...
function convertLocalToApiProject(p: Project, index: number): ApiProject {
  const images = [p.image, p.image1, p.image2, p.image3, p.image4, p.image5, p.image6]
    .filter((url): url is string => !!url)
    .map((url) => ({ url, publicId: "" }));

  const rawItems = [p.description1, p.description2, p.description3, p.description4, p.description5]
    .filter((s): s is string => !!s);

  const descriptionItems = rawItems.map((s) => {
    const clean = s.replace(/^🔴\s*/, "");
    const colonIdx = clean.indexOf(":");
    if (colonIdx === -1) return { label: "", value: clean };
  ...
  return {
    ...
    title: p.title,
    count: p.Count,
    status: p.status as "landscape" | "portrait",
    images,
    video: p.video,
    link: p.link,
...
const fallbackProjects: ApiProject[] = localProjects.map(convertLocalToApiProject);

const CATEGORIES = ["All", "Web", "Mobile", "Desktop"] as const;
type Category = (typeof CATEGORIES)[number];

function getProjectCategory(project: ApiProject): "web" | "mobile" | "desktop" {
  if (project.status === "portrait") return "mobile";
  const allDesc = project.descriptionItems
...
function parseTechBadges(project: ApiProject): string[] {
  const techs: string[] = [];
  for (const item of project.descriptionItems) {
...
function getImages(project: ApiProject): string[] {
...
interface ProjectCardProps {
  project: ApiProject;
  idx: number;
  onOpen: (project: ApiProject) => void;
}
...
const ProjectComponent: React.FC = () => {
  const { data: apiData, isLoading, isError } = useGetProjectsQuery();
  const data = (isError || !apiData) ? fallbackProjects : apiData;
  ...
  const openModal = (project: ApiProject) => {
